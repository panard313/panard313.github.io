...
可以发现，最终也是调用f_op的aio_read函数，对应于文件系统的文件就是xfs_file_aio_read函数，对应于块设备文件就是blkdev_aio_read函数，然后进入通用块层，放入IO队列，进行IO调度。由此可见Libaio的队列也就是通用块层之下的IO调度层中的队列。
...
所有到通用块层的IO，都要把数据封装成bio_vec的形式，放到bio结构体内。
